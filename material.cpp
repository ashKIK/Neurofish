...
namespace {
  ...
  template<Color Us>
  bool is_KBPsK(const Position &pos) {
...
private:
  ...
  template<typename T>
  const map<Key, T *> &get() const;
};

// Explicit specializations of a member function shall be declared in
// the namespace of which the class template is a member.
template<>
const EFMap &EndgameFunctions::get<EF>() const { return maps.first; }

template<>
const SFMap &EndgameFunctions::get<SF>() const { return maps.second; }
...
MaterialInfo *MaterialInfoTable::get_material_info(const Position &pos) {
  ...
  for (c = WHITE, sign = 1; c <= BLACK; c++, sign = -sign) {
    ...
    if (pieceCount[c][ROOK] >= 1)
      matValue -=
          sign * ((pieceCount[c][ROOK] - 1) * RedundantRookPenalty + pieceCount[c][QUEEN] * RedundantQueenPenalty);
