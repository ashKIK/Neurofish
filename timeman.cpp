...
void TimeManager::init(int myTime, int myInc, int movesToGo, int currentPly) {
  ...
  */
  ...
  for (hypMTG = 1; hypMTG <= (movesToGo ? Min(movesToGo, MoveHorizon) : MoveHorizon); hypMTG++) {
    ...
    hypMyTime = Max(
        myTime + (hypMTG - 1) * myInc - emergencyBaseTime - Min(hypMTG, emergencyMoveHorizon) * emergencyMoveTime, 0);
...
namespace {
  ...
  int remaining(int myTime, int movesToGo, int currentPly) {
    ...
    float ratio2 =
        (thisMoveImportance + TStealRatio * otherMovesImportance) / float(thisMoveImportance + otherMovesImportance);
