...
// Evaluation weights, initialized from UCI options
  enum {
    Mobility, PawnStructure, PassedPawns, Space, KingDangerUs, KingDangerThem
  };
  Score Weights[6];
  ...
  if (threads > 0) {
    ...
    switch (Options["Mode"].value<int>()) {
      case 0:
        ...
        break;
      case 1:
        bpn0 = new BPN(Options["Neural Network File"].value<std::string>().c_str(),
                       Options["Training Threads"].value<int>() - 1);
        break;
      case 2:
...
void read_evaluation_uci_options(Color us) {
  ...
  Weights[PawnStructure] = weight_option("Pawn Structure (Middle Game)", "Pawn Structure (Endgame)",
                                         WeightsInternal[PawnStructure]);
  Weights[PassedPawns] = weight_option("Passed Pawns (Middle Game)", "Passed Pawns (Endgame)",
