...
void BPN::constructor(unsigned *sizes, bool *biases, outFunction *functions,
                      unsigned layersNumber, double et, double alph,
                      double o_factor, int thrs) {
  ...
  if (size > 0) {
    ...
    layers[0] = new bpnLayer(sizes[0], 0, false, functions[0],
                             max_threads + 1); //  input layer - no lower layer
...
bool BPN::SaveToFile(const char *file) {
  ...
  try {
    ...
    fout << " init_scale: " << initial_scale << " eta: " << eta
         << " alpha: " << alpha << " scale_factor: " << scale_factor
         << " layers_num: " << size;
    
    for (i = 0; i < size; ++i) {
      bpnLayer *l = layers[i];
      fout << " layer" << i + 1 << ": size " << l->size << " bias " << l->bias
           << " func " << (int) l->func;
    }
...
void BPN::PrepareFromFEN(const char *fen, unsigned threadid, bool training) {
  ...
  if (fen[j] == 'w' || fen[j] == 'W')
    prods[256] = 1.0f; //	set active colour bit - i == 256
  else
    prods[256] = 0.0f;
...
void *BPN::UnitThreadFuncBias(void *arg) {
  ...
  for (unsigned j = lt->start; j < end;
       ++j) { //  for each unit - compute product
    net = 0;
...
bool BPN::DoThreading(unsigned layer, double (*apply)(double),
                      void *(*thrFunc)(void *)) {
  ...
  if (nthreads > 0 &&
      chunk < minchunk) { // decrease number of threads for bigger chunks
...
void BPN::Run() { //  assume that input is already placed in the first layer
  ...
  for (unsigned i = 1; i < size - 1; ++i) { //  for each layer above input
    //  determine output function before iterating through units - it's always
    //  same function for the whole layer
...
void BPN::Run(unsigned threadid) { //  assume that input is already placed in
  //  the first layer
  unsigned j, k;
...
bool BPN::Train() {           // threaded version
  ...
  // lower layers
  for (i = hidden_size; i > 0; --i) { //  compute errors and weights' changes -
    //  itterate layers from top to bottom
    //  determine derivate function before iterating through units - it's always
    //  same derivate for the whole layer
    switch (layers[i]->func) {
      case sigmoid:
        derivate = DerivateSigmoid;
        break;
...
bool BPN::Train() {
  ...
  for(i=size-2; i>0; --i) {   //  compute errors and weights' changes - itterate
    ...
    for(j=0; j < sizeL; ++j) {  //  for each unit in the layer
      ...
      try {
        ...
        for(k=0; k < lowerSize; ++k) {
          ...
          //layers[i]->weights[j][k] += layers[i]->deltas[j][k];  //  can't do
it here because of the lower layer errors
        }
      }
      catch(std::exception ex) {
        return false;
...
double BPN::ApplyLinear(double val) { return val; }

double BPN::ApplySigmoid(double val) { return (double) 1 / (1 + exp(-val)); }

double BPN::ApplySigmoid2(double val) {
  return (double) 2 / (1 + exp(-2 * val)) - 1;
}

double BPN::DerivateLinear(double) { return 1; }

double BPN::DerivateSigmoid(double val) { return val * (1 - val); }

double BPN::DerivateSigmoid2(double val) { return (double) 1 - (pow(val, 2)); }
